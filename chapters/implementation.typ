...
Implementace proto rozlišuje přihlášení prostřednictvím SSO, globální roli a vazbu ke konkrétnímu zdroji. Přínosem je jemnější řízení přístupů a menší rozsah zpřístupnění osobních údajů (NF01). Kompromisem je vyšší složitost autorizačního modelu.
...
Konkrétním integračním problémem bylo, že veřejně popsané `SOAP/WSDL` rozhraní neodpovídalo plně rozhraní, které bylo ve skutečnosti zpřístupněno pro provozní použití. V praxi se tak rozcházel očekávaný způsob napojení s reálně dostupnou vrstvou nad `InterSystems IRIS`, včetně konkrétních metod pro dotaz podle čísla pracovníka a rodného čísla. Součástí řešení proto bylo i doplnění potřebné `WSDL` popisné vrstvy a mapování metod na straně `IRIS`, aby bylo možné službu volat konzistentně a bez přenášení těchto odchylek do aplikační vrstvy. Přímé napojení backendu na generovaného SOAP klienta by totiž přeneslo nestabilitu integračního rozhraní do doménové logiky.
...
Tento mezistupeň pak řeší tři problémy najednou. Izoluje specifika skutečně dostupného integračního rozhraní mimo byznysovoulogiku, sjednocuje chybové stavy do kontrolovaného HTTP kontraktu a zjednodušuje případnou výměnu integračního detailu bez zásahu do domény.
