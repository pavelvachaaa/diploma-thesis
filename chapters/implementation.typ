...
První implementační technikou je promítnutí hexagonálního uspořádání, zdůvodněného v návrhu struktury backendu v @sec:arch-backend-structure, do konkrétního kódu. V kontextu #abbr("KZ", none) tento přístup zajišťuje, že rozhodování o náboru nebo vstupní agendě zůstává technologicky agnostické, což podporuje předatelnost verzovaného a dokumentovaného zdrojového kódu požadovanou v NF08. Správnost rozdělení ověřuji architektonickými testy hlídajícími čistotu importů mezi jádrem, porty a adaptéry. Cenou za toto oddělení je vyšší počet explicitních kontraktů a nutnost mapování dat mezi vrstvami.
...
Na @obr:candidate-to-employee je vidět detail uchazeče. Náborář na první pohled vidí všechny identifikační údaje, má rychlý přístup k dokumentům, poznámkám, a analytickým výstupem nad životopisem. Dále má možnost využít akce, které posouvají uchazeče v procesu, například odeslání e-mailu, naplánování pohovoru, zamítnutí nebo schválení a vytvoření zaměstnance.
