...
V předložené implementaci se hexagonální architektura projevuje striktním rozdělením systému na primární adaptéry a izolované doménové jádro. Primární adaptéry, umístěné na vstupní hranici aplikace, zahrnují HTTP routy, controllery, middleware pipeline a konzumenty integračních událostí. Jejich výhradní úlohou je přijmout vnější požadavek, extrahovat nezbytná data a předat řízení konkrétnímu případu užití (use-case) uvnitř domény. Samotné doménové jádro, soustředěné ve složce `src/domain`, je plně odstíněno od technologických detailů infrastruktury a veškerou svou byznysovou logiku vykonává bez jakékoliv přímé závislosti na vnějších systémech či komunikačních protokolech.

Vůči svému okolí doména komunikuje výhradně prostřednictvím explicitních kontraktů (portů) definovaných ve složce `src/shared/contracts/ports`, jejichž neměnnost a přesné hranice zajišťuje pomocná metoda `createServicePort`. Implementaci těchto odchozích závislostí následně realizují sekundární adaptéry umístěné ve složce `src/platform`. Moduly jako `platform/qualification` či `platform/userSearch` transformují doménové požadavky na interní HTTP volání, zatímco `platform/audit` a `platform/outbox` zajišťují bezpečný transport a spolehlivé doručení vedlejších efektů. Díky tomuto uspořádání pracuje doména výlučně se stabilními kontrakty, aniž by musela zohledňovat, zda je fyzická vrstva realizována prostřednictvím SQL, HTTP, AMQP nebo objektového úložiště.
