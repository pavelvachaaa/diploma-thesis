...
Přínosem pro #abbr("KZ", none) je reprodukovatelnost, dohledatelnost a lepší předatelnost řešení, což přímo podporuje udržitelnost podle NF08. Kompromisem je nutnost navrhovat i přechodové stavy, aby nová verze aplikace, dlouho běžící workery a existující data zůstaly kompatibilní.
...
Tím se podporuje ochrana osobních údajů (NF01) i auditovatelnost změn klíčových entit (NF11). Kompromisem je vyšší provozní složitost, protože je nutné sledovat nejen stav doménových záznamů, ale také stav navazujících infrastrukturních procesů.
...
Implementace proto rozlišuje přihlášení prostřednictvím #abbr("SSO", none), globální roli a vazbu ke konkrétnímu zdroji. Přínosem je jemnější řízení přístupů a menší rozsah zpřístupnění osobních údajů (NF01). Kompromisem je vyšší složitost autorizačního modelu.
