...
Volba `Node.js` a `Express` vychází z charakteru hlavního backendu. Jádro systému je převážně HTTP API s významným podílem middleware logiky, validace vstupů, integračních volání a asynchronního předávání událostí. `Express` zde poskytuje dostatečně tenkou aplikační vrstvu: nevnucuje vlastní doménový model, umožňuje přímo skládat routy, middleware a adaptéry a dobře zapadá do hexagonálního uspořádání popsaného v návrhu architektury. V porovnání s uceleným frameworkem typu `NestJS` jsem tak získal větší kontrolu nad tím, kde končí framework a kde začíná doménová logika. Alternativou byl jiný backendový stack, například `Spring Boot` či `ASP.NET Core`; ty by však v daném rozsahu přinesly více konvencí a vyšší implementační režii bez zřetelného přínosu.

Webové portály jsem postavil na `Next.js`. Vedle technických vlastností zde hrála důležitou roli i vyzrálost ekosystému. Široká komunita, dobře dostupná dokumentace a průběžný vývoj frameworku snižují riziko, že se řešení dostane do méně podporovaného technologického směru s horší udržovatelností. Současně jde o přirozené rozšíření knihovny `React`, takže bylo možné využít známý vývojový model i rozsáhlé know-how. Alternativou byla čistě klientská aplikace v `React` bez serverového vykreslení nebo řešení v jiném ekosystému, například `Nuxt`. Zvolený přístup však lépe podporoval dlouhodobou udržitelnost i sjednocení obou portálů v jednom technologickém základu.
...
Skládání těchto částí řídí dependency injection (DI) kontejner `Awilix`. V této části je důležitá hlavně jeho praktická role v kompozičním kořeni aplikace: propojuje aplikační služby s konkrétními adaptéry a vytváří request scope, ve kterém lze v rámci jednoho požadavku sdílet identitu uživatele nebo databázovou transakci napříč vrstvami. Doménový kód přitom zůstává bez závislosti na DI frameworku.
...
Skládání závislostí v modulární aplikaci řeším pomocí *Dependency Injection* s kontejnerem `Awilix`. Ruční propojování služeb a adaptérů by při desítkách komponent vedlo k nepřehlednému kompozičnímu kódu a obtížnému testování. `Awilix` zde navazuje na volbu `Express`: framework neřídí vnitřní architekturu aplikace a kompoziční kořen tak může explicitně propojit porty, adaptéry a aplikační služby. Funkčnost DI ověřuji testy startu aplikace (smoke tests). Omezením je, že část chyb v konfiguraci vazeb se projeví až za běhu systému.
