...
Volba Node.js a Express vychází z charakteru backendu. Jádro systému je převážně I/O orientované #abbr("HTTP", none) #abbr("API", none) s častými integračními voláními a asynchronním předáváním událostí, což dobře odpovídá neblokujícímu modelu Node.js.

Express zde slouží jako minimální #abbr("HTTP", none) vrstva na zpracování požadavků. Nevnucuje vlastní doménový model, umožňuje přímo skládat cesty, middleware a adaptéry a zapadá tím do hexagonálního uspořádání. Oproti uceleným frameworkům, jako je NestJS, poskytuje větší kontrolu nad hranicí mezi frameworkem a doménovou logikou. Alternativní možnosti, například Spring Boot či ASP.NET Core, by v daném rozsahu přinesly více konvencí a vyšší implementační režii bez zřetelného přínosu.
...
Volba jazyka Go u procesorů nebyla motivována dostupností knihoven pro práci s modely. Samotná inference je delegována do služby Ollama a extrakce textu do Apache Tika, obě dostupné přes #abbr("HTTP", none). Z funkčního hlediska by proto bylo možné tyto komponenty implementovat i v Node.js, což by sjednotilo technologický stack. Rozhodující byly jejich provozní vlastnosti. cv_processor běží jako dlouhožijící worker s více souběžnými konzumenty fronty a job_processor jako samostatná #abbr("HTTP", none)/#abbr("SSE", "Server-Sent Events") služba. Go v tomto kontextu nabízí jednoduchý model souběžnosti, malé samostatně nasaditelné binární soubory a předvídatelné chování u specializovaných služeb, které neřeší rozsáhlou doménovou logiku ani uživatelské rozhraní.

Vedle hlavního backendu řešení zahrnuje i samostatné služby pro auditní zápis, integrační adaptéry a inteligentní zpracování dokumentů či textů pracovních pozic. Tyto komponenty spolu komunikují přes #abbr("HTTP", none) nebo přes `RabbitMQ`. `RabbitMQ` jsem zvolil proto, že umožňuje spolehlivě oddělit transakční cestu od vedlejších efektů, aniž by bylo nutné zavádět robustnější streamovací platformu. Alternativou bylo čistě synchronní volání mezi službami nebo platforma typu `Kafka`. První varianta by zhoršovala odezvu a odolnost systému, druhá by v tomto měřítku přinesla spíše vyšší provozní režii.
...
Zpracování požadavku začíná v routovací vrstvě, kde middleware zajišťuje autentizaci a vytvoření kontextu požadavku. Vstupní adaptéry následně převádějí #abbr("HTTP", none) vstup na aplikační modely a volají aplikační službu nebo případ užití (use-case). Pokud tato logika potřebuje komunikovat s okolím, například s perzistencí, auditem nebo vrstvou zpráv, vyjadřuje tuto potřebu portem. Konkrétní technologické napojení pak poskytuje výstupní adaptér.

Skládání těchto částí řídí dependency injection (#abbr("DI", "Dependency Injection")) kontejner Awilix. V této části je důležitá zejména jeho role v kompozičním kořeni aplikace. Propojuje aplikační služby s konkrétními adaptéry a zajišťuje oddělený kontext pro každý požadavek, ve kterém lze v rámci jednoho požadavku sdílet identitu uživatele nebo databázovou transakci napříč vrstvami. Doménový kód přitom zůstává bez závislosti na #abbr("DI", none) frameworku.
...
#figure(
  [
    ...
    #table(
      ...
      [Vstupní adaptéry],
      [`src/adapters/in/http/` - routy a controllery převádějící #abbr("HTTP", none) požadavky na volání aplikačních služeb.],
      ...
      [Doménové moduly],
      [`src/core/*/domain/` - doménové objekty, pravidla a validace nezávislé na #abbr("HTTP", none), databázi a integračních službách.],
      ...
      [Výstupní adaptéry],
      [`src/adapters/out/persistence/`, `src/adapters/out/integration/` a `src/platform/` - konkrétní napojení na PostgreSQL, audit, outbox, #abbr("ReBAC", none), soubory, e-mail a externí služby.],
...
Poslední oblastí je integrace s vnějšími systémy, například registry nebo službami inteligentního zpracování dat, které mají cizí datové modely a nespolehlivá rozhraní. Standardní možností by bylo přímé volání těchto služeb z domény, což by ji však "znečistilo" cizími detaily. Použil jsem vzor `Anti-Corruption Layer` (#abbr("ACL", "Anti-Corruption Layer")). V implementaci napojení na registr kvalifikací #abbr("NRZP", none) tak doména pracuje se svými typy, zatímco adaptér řeší specifika #abbr("SOAP", "Simple Object Access Protocol") rozhraní. Tento přístup chrání stabilitu jádra systému. Správnost ověřuji mockovanými integračními testy. Tato ochrana však vyžaduje dodatečnou práci s překladem datových struktur.

Backend tak nestojí na jednom izolovaném vzoru, ale na jejich součinnosti. Hexagonální uspořádání chrání doménu, `Repository` izoluje databázi, `Dependency Injection` řídí skládání, `Transactional Outbox` s `Idempotency` stabilizují zápisy a #abbr("ACL", none) odděluje cizí systémy. Teprve v této kombinaci naplňuje implementace nároky na moderní platformu v prostředí #abbr("KZ", none).


=== Implementace doménové vrstvy
Smyslem doménové vrstvy není vytvořit další technickou mezivrstvu mezi controllerem a databází, ale soustředit pravidla personálního procesu do místa, které není závislé na #abbr("HTTP", none) rozhraní, konkrétním databázovém schématu ani integračních službách. V řešeném systému se tato pravidla týkají zejména životního cyklu pracovní pozice, uchazeče, pohovoru, nástupu zaměstnance, organizační příslušnosti a oprávnění k jednotlivým zdrojům.

Alternativou by bylo pojmout backend jako převážně CRUD aplikaci, kde by controllery přijímaly požadavky, repozitáře ukládaly data a většina pravidel by vznikala až jako podmínky v jednotlivých handler funkcích. Tento přístup je rychlý v počáteční fázi vývoje, ale v systému propojujícím nábor, vstupní agendu, autorizaci, audit a integrace by vedl k rozptýlení rozhodování o doménových pravidlech. Stejné pravidlo by se snadno objevilo na více místech a bylo by obtížné doložit, která část systému je za něj skutečně odpovědná.

Doménovou logiku proto člením do skupin podle významových oblastí systému, nikoli podle čistě technických operací. Patří sem zejména uchazeči, pracovní pozice, pohovory, zaměstnanci, dokumenty, onboarding, autentizace, notifikace, organizace, číselníky, oprávnění, kvalifikace, provozní pohledy a kontaktní dotazy. Případ užití (use-case) zde funguje jako aplikační scénář, který kontroluje vstupní podmínky, načte potřebné objekty, provede změnu a případně vytvoří požadavek na vedlejší efekt.
...
Pro #abbr("KZ", none) z toho plyne srozumitelnější a udržitelnější implementace procesů, které se budou pravděpodobně dál vyvíjet podle interních předpisů, organizačních změn a požadavků jednotlivých závodů. Změna pravidla v oblasti náboru nebo vstupní agendy tak nemusí znamenat zásah do prezentační vrstvy, integračních adaptérů ani auditního mechanismu. Cena za tuto volbu spočívá ve vyšší počáteční pracnosti. Vývojář musí udržovat hranice modulů, převádět data mezi vrstvami a odolávat pokušení obcházet doménu přímým zápisem do databáze.

=== Vynucení architektonických pravidel
Architektonická pravidla nestačí pouze deklarovat. Proto jsem je v implementaci částečně vynutil testy architektury. Ty ověřují, že doménové moduly neimportují repozitáře jiných modulů napřímo, že služby nepoužívají infrastrukturu mimo definované porty a že vedlejší efekty nejsou volány mimo outbox handlery.

Tento mechanismus je důležitý zejména proto, že architektonická kázeň má tendenci upadat právě v okamžiku, kdy projekt roste a přibývá tlak na rychlé úpravy. Testy zde fungují jako technická pojistka proti tomu, aby se výjimka z pravidla stala novým standardem.

=== Testování backendu
Backend pracuje s unit a integračními testy. Cílem nebylo dosáhnout formálně stoprocentního pokrytí kódu, ale pokrýt kritické části systému, u nichž by chyba mohla vést k nekonzistentnímu stavu procesu, porušení autorizace nebo ztrátě důležité vedlejší události.

Unit testy proto nahrazují produkční závislosti kontrolovanými implementacemi portů, například repozitářů, auditního zápisu, e-mailové služby nebo vrstvy předávání zpráv. Díky tomu lze ověřit vlastní rozhodování služby bez nutnosti spouštět databázi, `RabbitMQ` nebo externí integrační systémy. Tento přístup přímo navazuje na hexagonální architekturu. Jelikož doména komunikuje s okolím přes porty, lze v testu nahradit vnější svět a soustředit se na pravidlo, nikoli na infrastrukturu.

Integrační testy doplňují tuto rychlou vrstvu tam, kde je potřeba ověřit skutečnou spolupráci s perzistencí nebo sestavenou aplikací. Typicky jde o repozitáře, transakční chování, kontrakty služeb a průchod aplikační logiky přes reálnější infrastrukturu. Tím se snižuje riziko, že jednotkově správná služba selže až při napojení na databázové schéma, transakční hranici nebo registrační konfiguraci #abbr("DI", none) kontejneru.
...
RabbitMQ zde slouží jako oddělovací vrstva mezi producentem a konzumentem událostí. Umožňuje oddělit hlavní transakční operaci od následného zpracování vedlejších efektů a izolovat selhání konzumentů. Výměnou za to systém přijímá sémantiku opakovaného doručení, což vyžaduje idempotentní návrh konzumentů.
...
#figure(
  [
    ...
    #table(
      ...
      fill: (x, y) => if y == 0 { rgb("#eeeeee") } else { white },
      stroke: 0.5pt + gray,
      [Typ v outboxu], [Implementační efekt],
...
Worker využívá dávkové zpracování, zamykání položek, řízené opakování a chybový stav pro neobnovitelné chyby. Přímé volání vedlejších efektů je omezeno na outbox handlery, zatímco doménové služby pouze zapisují požadavky do outboxu. Tím se minimalizuje riziko nekonzistence mezi databází a integrační vrstvou.

Verzování událostí pomocí suffixu v1 (a dalších verzí) umožňuje bezpečnou evoluci integračního rozhraní. Při změně struktury nebo sémantiky zprávy lze zavést novou verzi bez nutnosti okamžité změny všech konzumentů.
...
Tato architektura naplňuje požadavek na dlouhodobou dohledatelnost změn klíčových entit, aniž by zhoršovala odezvu zapisovacích operací. Audit lze navíc provozně sledovat jako samostatný datový tok a využít jej pro interní kontrolu, bezpečnostní kontrolu nebo doložení průběhu personálního procesu.

Nevýhodou tohoto řešení je krátké časové okno mezi vznikem události a jejím perzistentním zápisem. Auditní vrstva proto vyžaduje dohled nad stavem fronty, stářím nejstarší nezpracované zprávy, počtem opakovaných pokusů a zprávami v chybovém stavu. Řešení tak přesouvá část složitosti z uživatelské cesty požadavku do provozního sledování systému. V daném kontextu je tato cena přijatelná, protože chrání odezvu systému a zároveň zachovává požadovanou auditovatelnost.
...
Smyslem této vrstvy není automatizovat rozhodnutí o vhodnosti uchazeče, ale převést nestrukturované dokumenty a texty do strukturované podoby, se kterou může systém dále pracovat. Výstupy slouží jako podpůrný podklad pro náboráře, nikoli jako samostatné rozhodovací kritérium. Finální posouzení zůstává na odpovědném pracovníkovi.

Z návrhového hlediska bylo možné tuto funkcionalitu začlenit přímo do hlavního backendu. Takové řešení by však spojilo transakční jádro s výpočetně náročnými a hůře předvídatelnými úlohami, jako je extrakce textu, generativní zpracování nebo tvorba embeddingů. Alternativou bylo využití externí cloudové služby (např. OpenAI nebo Google AI Studio). Tato varianta by sice snížila provozní nároky, ale u citlivých personálních dat by zvyšovala regulatorní a bezpečnostní rizika spojená s jejich předáváním mimo organizaci.
...
První část tvoří služba pro zpracování životopisů. Po přijetí události načte dokument z úložiště, extrahuje z něj text (Apache Tika) a připraví strukturovaný výstup použitelný v náborové agendě. Součástí je také vytvoření embeddingu, který umožňuje sémantické porovnávání mezi životopisem uchazeče a textem pracovní pozice. Výsledky jsou vráceny zpět do backendu a uloženy v kontextu uchazeče.

Druhou část tvoří služba pro práci s pracovními pozicemi. První funkcí je interaktivní asistence při tvorbě a úpravě textu nabídky. V tomto scénáři je služba volána synchronně (#abbr("HTTP", none)/#abbr("SSE", none)), aby mohl uživatel pracovat s generovaným návrhem v reálném čase. Druhou funkcí je tvorba embeddingů pro sémantické vyhledávání, která probíhá asynchronně. Backend při změně pozice zapíše požadavek do outboxu a hlavní transakce se okamžitě uzavře. Služba následně na pozadí vytvoří vektorovou reprezentaci a uloží ji do databáze.
...
Zvolil jsem proto `PostgreSQL 17` jako primární databázovou platformu a rozšíření `pgvector` pouze tam, kde je potřeba sémantické porovnávání životopisů a pracovních pozic. Přínosem je jednotný zdroj pravdy pro transakční proces, organizační filtr, auditní události i outbox. Toto rozhodnutí podporuje ochranu osobních údajů (NF01), protože citlivá data a jejich vazby zůstávají pod jednotnou správou, a současně nasaditelnost v on-premise prostředí (NF07), protože nevzniká závislost na externí vektorové službě. Nevýhodou zůstává, že specializovaná vektorová databáze by mohla být vhodnější při výrazně větším objemu podobnostního vyhledávání.

Použití `jsonb` u proměnlivých formulářových struktur a `pgvector` u embeddingů je omezeno na oblasti, kde pružnější datový typ skutečně snižuje složitost modelu. V jádru procesu zůstává relační model s referenční integritou. Organizace tím získává jednodušší provozní správu a jasnější životní cyklus dat.

=== Fyzický datový model
Fyzický model odpovídá hlavním doménovým hranicím. Nábor je veden kolem vazby pracovní role, pracovní pozice, uchazeče a pohovoru, zatímco vstupní agenda odděluje šablonu adaptačního postupu od její konkrétní instance nad zaměstnancem.

Alternativou by byl plošší model s menším počtem tabulek a větším množstvím stavových polí. Jeho nevýhodou by byla nejasná odpovědnost záznamů a obtížnější rozlišení, zda daný údaj reprezentuje pravidlo procesu, nebo jeho konkrétní průběh.

Zvolené rozdělení proto zachovává doménovou sémantiku i ve fyzickém schématu. Organizační izolace je nesena přes klíčové entity a přístupový model je opřen o uživatele, role, členství v organizaci a oprávnění ke zdrojům. Přínosem je, že datová vrstva přímo podporuje multi-tenantní model #abbr("KZ", none) a omezuje riziko, že se autorizace bude řešit až dodatečně nad již načtenými daty. Cena za tuto přesnost je složitější dotazování a větší nárok na konzistenci relačních vazeb.
...
=== Migrace schématu
Migrace zajišťují řízenou evoluci databázového modelu. Alternativou jsou ruční změny schématu prováděné přímo v databázi nebo jednorázové skripty mimo životní cyklus aplikace. Takový postup je sice rychlý při prototypování, ale obtížně se audituje, špatně se opakuje a zvyšuje riziko, že aplikace poběží proti nekompatibilní verzi schématu.

Zvolené řešení používá verzované migrace jako kontrolovaný přechod mezi verzemi aplikace a databáze. Migrace určují, jak se datový model vyvíjí, v jakém pořadí a s jakou vazbou na nasazovanou verzi systému.

Verzované migrace zlepšují reprodukovatelnost, dohledatelnost a předatelnost řešení, což přímo podporuje udržitelnost podle NF08. Tato volba však vyžaduje navrhovat i přechodové stavy, aby nová verze aplikace, dlouho běžící workery a existující data zůstaly kompatibilní.
...
Vstupní agenda se tím mění v opakovatelný proces, nikoli v sadu ručně předávaných pokynů. HR pracovník získává místo pro správu pravidel a dokumentů, zatímco nastupující zaměstnanec v portálu pracuje pouze se svou konkrétní sadou úkolů.
...
Rozhraní vychází z ustálených vzorců kariérních portálů. Náhledová karta shrnuje klíčové atributy nabídky a filtrační panel zůstává stabilní, což umožňuje porovnávat více výsledků bez ztráty orientace. Cílem není pouze grafické řešení, ale snížení kognitivní zátěže při práci s větším množstvím podobných nabídek.
=== E2E testování portálu
Spolehlivost veřejného portálu je podpořena automatizovaným ověřením celé náborové cesty. Vzhledem k tomu, že portál představuje vstupní bod pro uchazeče, nestačí testovat pouze jednotlivé prvky uživatelského rozhraní. Klíčové je ověřit celý tok od vyhledání pracovní pozice přes otevření jejího detailu až po odeslání reakce s životopisem a zároveň tím prověřit integrační kontrakt mezi frontendem a transakčním #abbr("API", none).

Tento přístup nepředstavuje přímé měření dostupnosti ve smyslu NF04 (99,5 % v pracovních dnech mezi 6:00 a 22:00), ale snižuje riziko regresí v kritické veřejné náborové cestě. Dostupnost jako provozní vlastnost je následně sledována v dohledové vrstvě prostřednictvím healthchecků, metrik dostupnosti služeb a stavů integračních komponent.

Jako alternativu jsem mohl zvolit pouze komponentové nebo integrační testy s mockovaným #abbr("API", none). Ty by však neodhalily poruchy vznikající až při běhu sestaveného portálu proti skutečně spuštěnému backendu. Reálnou alternativou byl i `Cypress`, avšak pro tento projekt jsem zvolil `Playwright`, protože umožňuje spouštět scénáře nad sestavenou verzí portálu v reálném prohlížeči, dobře pracuje s navigací, nahráváním příloh a vícekrokovým formulářovým tokem a současně se přirozeně integruje do CI/CD pipeline, včetně opakování selhaných běhů a záznamu trasování při chybě.

Testovací scénáře obsahují zobrazení veřejného seznamu pozic, vyloučení neveřejných nabídek, filtrování a vyhledávání nad skutečným #abbr("API", none), přechod ze seznamu do detailu a dále na formulář reakce, odeslání přihlášky v režimu s povinným i nepovinným životopisem, klientskou validaci formulářů, samostatný kontaktní formulář i formulář pro zájemce bez vazby na konkrétní pozici.

 U klíčových scénářů nekončím na úrovni obrazovky, ale kontroluji i perzistenci výsledku v databázi, například vznik záznamu uchazeče, uložení přílohy nebo zápis kontaktního dotazu. Tímto postupem snižuji riziko, že po změně frontendu, #abbr("API", none) nebo validační logiky zůstane portál vizuálně dostupný, ale fakticky přestane spolehlivě přijímat použitelné reakce.
...
Nástroj Umami jsem zvolil především s ohledem na provozní charakter řešení. Jde o lehké řešení, které lze provozovat v on-premise režimu, nevyžaduje rozsáhlou konfiguraci a dobře podporuje měření účelově definovaných událostí. To odpovídá požadavku NF07 na nasaditelnost v infrastruktuře organizace.

V implementaci je analytický skript připojen centrálně a při on-premise nasazení je zpřístupněn přes vlastní proxy cestu. Analytická vrstva tak nenarušuje kontrolu nad veřejným rozhraním ani tokem dat mimo organizaci.

Alternativou byly robustnější platformy, například Google Analytics 4, které však přinášejí závislost na externí cloudové službě a širší funkční rozsah, než jaký byl pro portál potřebný. Další možností byl nástroj Matomo, který lze rovněž provozovat lokálně, ale za cenu vyšší provozní režie a složitější správy.
...
Z architektonického hlediska jsem proto integrační logiku neumístil přímo do backendu. Místo toho jsem ji navrhl jako  mezivrstvu, která odpovídá principům vzorů `Adapter` a `Anti-Corruption Layer`. Backend vystavuje pouze administrační endpoint `POST /api/v1/admin/qualifications/lookup`, který je dostupný vybraným rolím. Doménová služba podporuje dotaz podle čísla pracovníka v #abbr("NRZP", none) i podle rodného čísla, vstup normalizuje a validuje a teprve potom volá platformní adaptér.

Služba `qualification-adapter` běží pouze v interní Docker síti a funguje jako mezivrstva mezi backendem a integrační vrstvou nad `InterSystems IRIS`. Volba `IRIS` zde není náhodná. V prostředí #abbr("KZ", none) se tato platforma dlouhodobě používá jako integrační vrstva pro napojování okolních systémů, a proto bylo vhodnější navázat na existující provozní standard než zavádět pro jedinou vazbu samostatný integrační middleware. 

Tento mezistupeň pak řeší tři problémy najednou. Izoluje specifika skutečně dostupného integračního rozhraní mimo byznysovou logiku, sjednocuje chybové stavy do kontrolovaného #abbr("HTTP", none) kontraktu a zjednodušuje případnou výměnu integračního detailu bez zásahu do domény.
...
Požadavky F13–F15 jsou realizovány integračním napojením na #abbr("NRZP", none) přes oddělený qualification-adapter, který zajišťuje ověření odborné způsobilosti, sjednocení výsledku do doménového modelu a vytvoření auditovatelného záznamu. Vstupní agenda a adaptace (F16–F21) jsou pokryty samostatným portálem a doménovým modelem oddělujícím šablonu adaptačního postupu od konkrétní instance zaměstnance. Požadavky F22–F25 se promítají do multi-tenantního datového modelu, organizačních filtrů, rolí centrální správy a přehledových dat nad náborem a adaptací.

Ochrana osobních údajů (NF01) je podpořena on-premise zpracováním životopisů a inference, maskováním citlivých identifikátorů v auditu a vyhodnocováním přístupu prostřednictvím autorizačního modelu. Autentizace (NF02) je řešena přes #abbr("SSO", none) pomocí OpenID Connect (#abbr("OIDC", none)) nad #abbr("OAuth", none) 2.0 a navazující vytvoření aplikačního kontextu uživatele. Řízení přístupů (NF03) je realizováno kombinací globálních rolí a vztahových oprávnění nad konkrétními zdroji.

Požadavek na dostupnost (NF04) není prokazován testy, ale provozním dohledem popsaným v kapitole nasazení, kde jsou sledovány healthcheck endpointy, metriky dostupnosti a stav integračních komponent. Výkon (NF05) je podporován oddělením výpočetně náročné vrstvy od transakčního backendu, asynchronním zpracováním vedlejších efektů a oddělenou auditní vrstvou. Přístupnost (NF06) se týká především veřejného portálu. V aktuální verzi není požadavek #abbr("WCAG", none) 2.1 na úrovni AA splněn v plném rozsahu, portál je však plně responzivní a jeho hlavní scénáře jsou ověřovány v reálném prohlížeči.

Nasaditelnost (NF07) je podpořena volbou technologií provozovatelných v infrastruktuře organizace prostřednictvím kontejnerizace (PostgreSQL, SeaweedFS, RabbitMQ, Ollama). Udržitelnost (NF08) podporuje modulární struktura backendu, verzované migrace, architektonické testy a jasně definované hranice mezi doménou a adaptéry. Lokalizace (NF09) je zajištěna českým uživatelským rozhraním a podporou diakritiky ve všech vrstvách systému. Kompatibilita (NF10) je řešena volbou standardního webového stacku podporujícího aktuální verze hlavních prohlížečů. Auditovatelnost (NF11) je realizována asynchronní auditní vrstvou, která uchovává záznamy o změnách klíčových entit bez negativního dopadu na odezvu systému.
