...
== Modelování hrozeb 
Součástí dobré praxe při nasazení je i posouzení toho, jaké útoky nebo provozní chyby mohou překročit hranice mezi jednotlivými částmi systému. Modelování hrozeb zde používám v duchu Shostackova pojetí jako návrhovou aktivitu zaměřenou na hledání rizik a jejich mitigací už při návrhu systému @shostackThreatModeling2014. V této kapitole jej používám zjednodušeně. Nejedná se o úplný bezpečnostní audit všech komponent, ale jako praktické posouzení síťové segmentace a dopadu kompromitace jedné vrstvy na zbytek nasazení.
...
#figure(
  [
    ...
    #table(
      ...
      [Host-only binding portů dohledové vrstvy, řízení přístupu ke `Grafana` a omezení citlivých štíťků.],
    )
  ],
