...
import { useEmployeeChatApi } from '@/components/chat/useEmployeeChatApi'
...
function EmployeeChatPage() {
    ...
    // Load available HR users for new conversations
    useEffect(() => {
        loadAvailableHRUsers()
    }, [loadAvailableHRUsers])
    ...
    // If no thread exists (new conversation), get user info from availableHRUsers
    const selectedUserInfo = selectedThread || (selectedThreadId ? availableHRUsers.find(user => user.id === selectedThreadId) : null)
    ...
    // Filter available HR users for new chat (exclude existing threads)
    const availableNewChatUsers = (availableHRUsers || []).filter(user => {
            ...
            `}>
                ...
                {showNewChat ? (
                    <div className="flex-1 p-4">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="font-medium">Kontaktovat HR pracovníka</h3>
                            <Button
                                variant="ghost"
                                size="sm"
                            ...
                            <div className="space-y-2 max-h-96 overflow-y-auto">
                                {availableNewChatUsers.length === 0 ? (
                                    <div className="text-center py-8 text-gray-500">
                                        ...
                                        Všichni HR pracovníci již mají aktivní konverzaci
                                    </div>
                                ) : (
                ...
                {selectedThreadId ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-50">
                        ...
                        <p className="text-sm text-gray-500 mb-4">
                            Zvolte existující konverzaci nebo kontaktujte HR pracovníka
                        </p>
                        <Button
                            variant="outline"
                            onClick={() => setShowNewChat(true)}
...
export default function ProtectedEmployeeChatPage() {
    return (
