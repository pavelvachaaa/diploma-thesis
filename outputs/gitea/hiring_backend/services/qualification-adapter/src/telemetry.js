// OTel bootstrap for qualification-adapter — must be required before express.
...
const start = () => {
    ...
    try {
        ...
        // Telemetry libs not installed yet — silently skip.
        return;
    }

    const endpoint = process.env.OTEL_EXPORTER_OTLP_ENDPOINT || 'http://alloy:4318';
    const serviceName = process.env.OTEL_SERVICE_NAME || 'qualification-adapter';
    ...
    try {
        ...
        console.error('[otel] qualification-adapter failed to start SDK', error);
    }
